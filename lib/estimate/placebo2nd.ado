...
program define placebo2nd, rclass
    version 18
    
    syntax varlist(min=4 max=6 numeric) [if] [in], [cluster(varname) fixed_effects(varname)]
    
    * Parse varlist: outcome treatment treated_group X [cluster] [fixed_effects]
    gettoken outcome rest : varlist
    gettoken treatment rest : rest
    gettoken treated_group rest : rest
