...
revenue, return on assets, log labor productivity, etc. For each measure
$k$, we write
\begin{equation}
q_{it}^{k}
=
...
We impose strict exogeneity of firm-level shocks with respect to the entire CEO-spell history:
\begin{equation}
E\!\left[
e_{it}^{k}
\mid
\alpha_i^{k},
\{z_{is}^{k}\}_{s\in\mathcal S_i}, {s(i,r)}_r
\right]
=
0
\qquad
...
\end{equation}
This assumption requires the firm-level shock in each period to have
mean zero conditional on the firm's entire sequence of CEO-spell
effects and CEO-spell assignments. In particular, it rules out
systematic selection of CEO transition timing on realizations of the
firm-level shocks. It therefore implies that the average shock within
each CEO spell is zero in expectation, so that the difference between
average outcomes in two adjacent spells is an unbiased measure of the
...
spell changes. The distortion disappears only when the within-spell estimation error converges to zero: for example, as the number of observations in both spells grows under conditions that allow the firm-level shocks to average out. Otherwise, the estimated variance remains upward biased.
\vspace{.2cm}

\paragraph{Bias in the covariance.} The covariance entering the naive statistics is also affected by estimation error in the CEO-spell change. Using the decomposition of
...
One implication of the biased second moments is that the $R^2$ statistic
is also biased. This is an important measure in the literature on CEO
effects because it summarizes the explanatory power of the true
CEO-spell change for variation in the outcome contrast. The estimated naive
$R^2$ has the form
...
outcomes observed before the CEO arrives. This may produce spurious pre-trends as well as distorted post-transition event-study dynamics.
...
second moments of estimated CEO-spell changes. The estimated variance
of the CEO-spell change is upward biased by $B^k\geq 0$, while its
estimated covariance with the outcome contrast is distorted by
$A_h^{jk}$, which may be positive or negative.
...
We match each firm that moves from CEO spell $s-1$ to spell $s$ with a firm that is similar on observables and whose CEO does not change over the full period spanned by the treated firm's two adjacent CEO spells, from the first year of spell (s-1) through the last year of spell (s). We match firms on characteristics such as one-digit sector, birth cohort in bins of three years and whether maximum employment over the firm's lifetime is below 10 or at least 10.. We assign each matched firm a placebo transition in the same calendar year $g_{is}$ as the corresponding actual transition. This ensures that treated and placebo firms are compared over the same macroeconomic period. We divide placebo firms' uninterrupted CEO spell into artificial pre- and post-transition segments with the same lengths as the two spells in the corresponding treated firm. For notational simplicity, we label
...
This restriction requires, for each pair of outcomes \(j\) and \(k\), the temporal pattern of variances, autocovariances, and cross-outcome covariances to be the same in the treated and placebo groups up to the pair-specific scale factor \(c^{jk}\) or \(c^{k}\). The restriction therefore allows the magnitudes of shock variances and cross-outcome covariances to differ systematically between treated and placebo firms.
...
the matched-design construction, and consistency of $\widehat c^{jk}$ and $\widehat c^{k}$,
...
\begin{table}
    ...
    \input{\tablepath/debiased_r2s.tex}
    \begin{minipage}{10cm}
    \vspace{.2cm}
