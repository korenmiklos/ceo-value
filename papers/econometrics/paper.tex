...
\begin{equation}\label{eq:model1}
  y_{it} = z_{m(i,t)} + e_{it},\qquad \mathbb E[e_{it}|\{z_{is}\}_{s=1}^T]=0,
\end{equation}
where $z_{m(i,t)}$ collects the CEO effect at time $t$ and $e_{it}$ is a shock. We assume that $z_{m(i,t)}$ is piecewise constant, changing only when the CEO changes and that $e_{it}$ is mean independent of the CEO path for all $s$ (``strict exogeneity'').
...
To be clear: the point estimates of individual CEO fixed effects $\hat z_n$ are unbiased for the true effects $z_n$. The problem is that all second moments involving these estimates—variances, covariances, regression slopes, correlations—are biased. Because almost every applied question uses second moments (How much do CEOs contribute to firm variance? Are better CEOs associated with higher investment? Do outcomes improve after CEO arrival?), the small-sample noise in first moments propagates into bias in the quantities researchers care about.
...
For a general linear contrast, let $\Delta \hat z_i$ denote the estimated change in CEO effects for transition $i$, and $\Delta y_i$ the corresponding change in outcomes. We assume that the contrast weights sum to zero (such as $+1$ for $t=+3$ and $-1$ for $t=-1$), so that under strict exogeneity, the mean change in outcomes would equal the mean change in CEO effects: $\mathbb E[\Delta y_i|\Delta z_i] = \Delta z_i$. 

In applications with heterogeneous spell lengths and event windows, firms are grouped into design groups $g=1,\ldots,G$, each with $N_g$ transitions. The sample covariance and variance are
\begin{align}
...
The key insight is that for placebo transitions, there is no true CEO effect change: $\Delta z_i = 0$ for all placebo firms. Yet the placebo sample has the same spell-length distribution and event-window design as the treated sample. Therefore, the placebo moments recover the bias terms under the \emph{proportional autocovariance} assumption.
...
\begin{equation}
\widehat{\Cov}^{\,\text{pl}}(\Delta y_i,\Delta \hat z_i) \xrightarrow{p} \frac{1}{c} A,\qquad \widehat{\Var}^{\,\text{pl}}(\Delta \hat z_i) \xrightarrow{p} \frac{1}{c} B.
\end{equation}
...
\begin{align}
\widehat{\Cov}^{\,\text{db}}(\Delta y_i,\Delta \hat z_i) &= \widehat{\Cov}^{\,\text{tr}}(\Delta y_i,\Delta \hat z_i) - c\widehat{\Cov}^{\,\text{pl}}(\Delta y_i,\Delta \hat z_i),\\
\widehat{\Var}^{\,\text{db}}(\Delta \hat z_i) &= \widehat{\Var}^{\,\text{tr}}(\Delta \hat z_i) - c\widehat{\Var}^{\,\text{pl}}(\Delta \hat z_i).
\end{align}
...
The excess variance term $c$ can be estimated by comparing the overall variances of outcomes in treated and control groups in an event window where no CEO change occurs. In practice, [[ explain what we do]]
...
We estimate CEO spell effects but not firm fixed effects. Because all contrasts are computed relative to the year before CEO arrival (t=-1), any firm-level intercept would be differenced away. In principle, one could include firm fixed effects in a two-way specification as in the labor literature \citep{Abowd1999Econometrica}. However, our bias-correction method applies to any fixed-effect setup; we demonstrate it in the simpler one-way case for clarity. [[comment: confirm
we want to emphasize generality vs. specific TWFE]]
...
We present event studies examining revenue dynamics around CEO transitions (Figure \ref{fig:application}).
...
\footnotesize Notes: Panels: Baseline, Long Panel, Persistent Errors, Unbalanced Panel, Excess Variance, All Complications. Bands are truncated to [-0.75, 1.75] for readability. [[comment: add precise definitions for beta0, beta1, dbeta from the estimator]]}
\end{minipage}
\end{figure}
