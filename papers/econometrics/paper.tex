...
The key properties of the bias terms are:
\begin{itemize}
\item $A$ and $B$ are weighted sums over design groups, with weights determined by spell lengths and event-window contrasts.
\item When shocks are i.i.d. and spells are long, $A\approx 0$ and $B\approx 0$.
\item When spells are short or shocks are persistent, $A$ and $B$ can be large, severely distorting variances, covariances, and regression slopes.
\item The bias terms are \emph{the same} (up to a scalar multiplier) for any two samples that have identical spell-length distributions and event-window designs, even if one sample has no true CEO effects.
\end{itemize}

This last property is the foundation of our placebo-controlled debiasing.
