...
\title{How Much Do CEOs Matter? \\ Correcting for Small-Sample Bias in Fixed-Effect Estimates \\ OR \\ CEO Replacements and Firm Growth/Performance/Behavior: Correcting for Small-Sample Bias in Fixed-Effect Estimates\thanks{%
%% Krisztina, pls add thank
...
%% Miklos or Almos, we should justify n the below paragraph why we look at those complications as opposed to other ones
...
[[I do not understand something below: shouldn't CEOs go from one firm to another in order to estimate z?]]
\paragraph{Estimating CEO Spell Effects.} Consider a firm with two CEOs (A and B), with B replacing A. Take two time spells, which cover the entire tenure of A and B (not just the event window).\footnote{While we restrict the estimation of the CEO transition to the event window to be close to the transition year, we let the CEO effect be measured across as many years as possible.} We compute a spell-level CEO effect as the within-spell mean of firm revenue, which we measure relative to the industry-year mean. Let \(r_{it}\) denote firm revenue and let \(\tilde r_{it}=r_{it}-\bar r_{st}\) be the revenue demeaned by industry - year \((s,t)\) cells. Let \(1\{\text{spell}=s\}\) indicate spell A or B. The spell-level CEO effect is
\begin{equation}
...
\paragraph{Results.} Figure \ref{fig:application} presents the results of CEO changeWe present the naive (red line) and the debiased (blue line) event studies examining revenue dynamics around CEO transitions . 
