...
$t$. Such measures can include for example log
revenue, return on assets, and log labor productivity. For each measure
$k$, we write
\begin{equation}
q_{it}^{k}
=
...
component of firm performance associated with that CEO spell. The spell
component is constant during a CEO spell and changes when the firm
changes CEO. The remaining variation is captured by the firm-level
shock $e_{it}^{k}$.
...
Throughout the paper, we occasionally refer to it as a CEO effect for
brevity, but the empirical object is the component of firm performance
associated with a CEO spell at a particular firm.

Our interest is not in the level of the firm fixed effect. We therefore
difference outcomes across adjacent CEO spells within the same firm.
This removes $\alpha_i^{k}$ and allows us to study changes in CEO-spell
components without requiring a connected CEO-firm mobility network.

\subsection{Changes in CEO-spell components}
\label{subsec:spell_changes}

Let $\mathcal C$ denote the set of observed CEO transitions. Transition
$n\in\mathcal C$ occurs at firm $i(n)$ when the firm moves from CEO
spell 0 to CEO spell 1. Let $\mathcal T_{n0}$ and $\mathcal T_{n1}$
denote the sets of years belonging to the two spells, with respective
lengths $T_{n0}$ and $T_{n1}$.

The true change in the CEO-spell component measured using outcome $k$
is
\begin{equation}
\Delta z_n^{k}
=
...
We do not observe either spell component directly. We estimate their
difference using the difference between the average outcomes observed
during the two spells:
\begin{equation}
\Delta\widehat z_n^{k}
=
\frac{1}{T_{n1}}
\sum_{t\in\mathcal T_{n1}}
q_{i(n)t}^{k}
-
\frac{1}{T_{n0}}
\sum_{t\in\mathcal T_{n0}}
q_{i(n)t}^{k}.
...
\end{equation}
is the estimation error in the measured change in the CEO-spell
component.
...
\end{equation}
The estimated change is therefore unbiased. It is also consistent as
the number of observations in both spells becomes large. In typical
CEO-firm panels, however, CEO spells are short. As a result, the
averages in Equation~\eqref{eq:eta_definition} contain substantial
firm-level noise.

This is the source of the small-sample bias. Even when the number of
firms and CEO transitions is very large, the estimation error in an
individual CEO-spell change does not disappear if the number of
observations within each spell remains small. Increasing the number of
firms makes the distribution of the noisy estimates more precisely
measured, but it does not make the individual spell estimates less
noisy.

\subsection{Firm outcomes and CEO-spell changes}
\label{subsec:outcomes_spell_changes}

We study how a firm outcome changes when the firm moves from one CEO
spell to another. Let $y_{it}^{j}$ denote outcome $j$ of firm $i$ in
...
status changes around the transition.

Let $g_n$ denote the calendar year of transition $n$. For event time
$h$, define the outcome contrast
\begin{equation}
\Delta_h y_n^{j}
=
y_{i(n),g_n+h}^{j}
...
\end{equation}
Thus, outcomes in each event year are measured relative to the year
before the CEO transition. The same argument applies to more general
linear contrasts, including differences between average
post-transition and pre-transition outcomes.

The population projection coefficient of interest is
\begin{equation}
\beta_h^{jk}
=
\frac{
...
\end{equation}
This coefficient describes how outcome $j$ comoves with the change in
the CEO-spell component measured using variable $k$. Equivalently, we
may write
\begin{equation}
\Delta_h y_n^{j}
=
\beta_h^{jk}\Delta z_n^{k}
+
u_{nh}^{jk},
\label{eq:population_projection}
\end{equation}
where
\[
\operatorname{Cov}\!\left(
...
Because the true CEO-spell change is unobserved, the natural estimator
replaces $\Delta z_n^{k}$ with $\Delta\widehat z_n^{k}$:
\begin{equation}
...
Although $\Delta\widehat z_n^{k}$ is an unbiased estimate of
$\Delta z_n^{k}$, the second moments used in
Equation~\eqref{eq:naive_beta} are generally biased. Short-spell
estimation error inflates the variance of $\Delta\widehat z_n^{k}$ and
contaminates its covariance with the outcome contrast. Because the
regression coefficient is formed as the ratio of these two biased
moments, it is generally biased as well.

\subsection{Bias in the variance}
\label{subsec:variance_bias}

Using Equation~\eqref{eq:spell_change_error}, the variance of the
estimated CEO-spell change is
\begin{align}
\operatorname{Var}\!\left(
\Delta\widehat z_n^{k}
\right)
...
\end{equation}
is the variance bias.

The observed dispersion in estimated CEO-spell changes therefore
combines genuine heterogeneity in spell changes with the dispersion
generated by firm-level shocks. A variance decomposition based directly
on $\Delta\widehat z_n^{k}$ will overstate the importance of CEO-spell
changes.

The magnitude of $B^{k}$ depends on the lengths of the two spells and
on the temporal covariance structure of firm-level shocks. Short spells
generate larger estimation error because each spell mean is calculated
from fewer observations. Persistent shocks can further increase or
otherwise alter the bias because shocks within and across the two spells
are correlated.

The variance bias does not disappear as the number of firms grows. If
spell lengths remain bounded, then as the number of transitions tends
to infinity,
\begin{equation}
\widehat{\operatorname{Var}}\!\left(
\Delta\widehat z_n^{k}
...
\end{equation}
The sample variance converges precisely to the wrong population object.
The bias disappears only as the amount of information within CEO spells
becomes sufficiently large for $\eta_n^{k}$ to converge to zero.

\subsection{Bias in the covariance}
\label{subsec:covariance_bias}

The numerator of Equation~\eqref{eq:naive_beta} is also contaminated by
the estimation error in the CEO-spell change. Using
Equations~\eqref{eq:spell_change_error} and
\eqref{eq:population_projection},
\begin{align}
\operatorname{Cov}\!\left(
\Delta\widehat z_n^{k},
\Delta_h y_n^{j}
...
\end{align}
Here, the definition of the population projection implies that
$\Delta z_n^{k}$ is uncorrelated with $u_{nh}^{jk}$, while strict
exogeneity implies that $\Delta z_n^{k}$ is uncorrelated with
$\eta_n^{k}$.
...
\end{equation}
The covariance based on the estimated CEO-spell change is then
\begin{equation}
\operatorname{Cov}\!\left(
\Delta\widehat z_n^{k},
\Delta_h y_n^{j}
...
The term $A_h^{jk}$ is the covariance bias. It arises because the
firm-level shocks used to estimate the CEO-spell change may also enter,
or be correlated with shocks entering, the outcome contrast.

The source of this problem is the short length of CEO spells. Each
CEO-spell component is estimated from only a small number of annual
observations, so firm-level shocks do not average out. Because the
estimated explanatory variable is constructed from the firm's own
realized outcomes over these short spells, the same shocks can enter
both the measured CEO-spell change and the outcome contrast. The
estimation error in $\Delta\widehat z_n^{k}$ can therefore be
mechanically correlated with the outcome being studied.
...
variable contaminates the denominator but is assumed to be independent
of the dependent variable. Here, short-spell estimation error can
contaminate both the denominator and the numerator.

Substituting Equations~\eqref{eq:variance_bias} and
\eqref{eq:covariance_bias} into Equation~\eqref{eq:naive_beta}, the
probability limit of the naive estimator is
\begin{equation}
\operatorname{plim}
\widehat\beta_{h,\mathrm{naive}}^{jk}
=
...
Both the numerator and denominator are biased. The direction of the
resulting bias in the regression coefficient is therefore ambiguous.
If the covariance and variance happen to be inflated in the same
proportion, the two biases may cancel in the ratio. This is a special
case, however, and does not generally hold when firm-level shocks are
persistent or when different outcome contrasts are considered.

\subsection{Dynamic bias and spurious pre-trends}
\label{subsec:dynamic_bias}

The covariance bias is particularly important in event-study
...
shocks are serially correlated, shocks used to estimate the incoming
CEO's spell component may be correlated with firm outcomes observed
before the CEO arrives.

To see the problem, consider a negative or positive shock that begins
before the CEO transition and persists into the post-transition spell.
Because the shock affects the outcomes observed under the incoming CEO,
it enters $\eta_n^{k}$. Because it began before the transition, it is
also correlated with pre-transition outcomes. The estimated change in
the CEO-spell component can therefore appear to predict outcomes before
the CEO arrives, even though the true spell component has not yet
changed.

For a pre-transition event time $h<0$, the true coefficient may satisfy
\[
\beta_h^{jk}=0.
\]
Nevertheless, Equation~\eqref{eq:naive_beta_plim} becomes
\begin{equation}
\operatorname{plim}
\widehat\beta_{h,\mathrm{naive}}^{jk}
=
...
\end{equation}
which need not be zero. Persistent shocks can thus generate spurious
pre-trends mechanically.

The same problem distorts post-transition dynamics. A true effect that
occurs immediately may appear to build gradually, decline over time, or
overshoot, depending on how the estimation error in the CEO-spell
change covaries with outcomes at different event times. Observed
dynamics may therefore reflect the covariance structure of firm-level
shocks rather than the timing of the change associated with the CEO
spell.

\subsection{Implications for commonly reported statistics}
\label{subsec:implications}

The short-spell estimation error affects any statistic constructed from
second moments of estimated CEO-spell components.

First, the variance of estimated CEO-spell changes overstates genuine
heterogeneity in the changes experienced by firms:
\[
\operatorname{Var}\!\left(
\Delta\widehat z^{k}
\right)
...
Second, covariances between estimated CEO-spell changes and firm
outcomes contain the additional term $A_h^{jk}$. This affects
conclusions about which firm policies or outcomes are associated with
changes in the CEO-spell component.

Third, regression coefficients inherit both the variance and
covariance biases through the ratio in
Equation~\eqref{eq:naive_beta_plim}. The resulting coefficient may be
biased upward or downward.

Fourth, correlations and $R^2$ statistics are nonlinear functions of
estimated variances and covariances. Debiasing only one component is
therefore insufficient: the underlying second moments must be corrected
before constructing these statistics.

Finally, event-study coefficients can display spurious pre-trends and
distorted post-transition dynamics even when
Equation~\eqref{eq:strict_exogeneity} holds. A conventional pre-trend
test may therefore reject a valid empirical design because of the
mechanical relationship between the estimated CEO-spell change and
...
Recovering the relationship between CEO-spell changes and firm outcomes
requires identifying and removing both $A_h^{jk}$ and $B^{k}$. In the
next section, we show how matched firms that do not change CEOs can be
used to estimate these bias terms.
```


\section{Placebo-Controlled Debiasing} \label{sec:placebo_debiasing} Our approach constructs placebo CEO transitions to identify and remove the bias terms $A_h^{jk}$ and $B^k$ without specifying the full covariance structure of firm-level shocks. The key insight is that the short-spell bias depends on two objects: the design used to construct the estimated CEO-spell change and outcome contrast, and the second-order dependence structure of the shocks entering these objects. A sample with the same design but no true CEO-spell change can therefore identify the bias under an appropriate restriction on the shock process. \paragraph{Construction of the placebo sample, identification restriction.} For each firm experiencing an actual CEO transition, we select one or more observably similar firms whose CEO does not change over the relevant event window. We match firms on characteristics such as sector and birth cohort. We assign each matched firm a placebo transition in the same calendar year as the actual transition and divide its uninterrupted CEO spell into artificial pre- and post-transition spells with the same lengths as in the corresponding treated firm. We then apply exactly the same transformations to the treated and placebo observations. In particular, we use the same pre- and post-spell lengths to construct $\Delta\widehat z_n^k$, the same event-time contrasts to construct $\Delta_h y_n^j$, and the same weights when aggregating across designs. Assigning the placebo transition in the same calendar year also ensures that treated and placebo firms are exposed to the same aggregate time period. Matching does not by itself identify the correction. Its purpose is to reproduce the treated sample's design and to make it plausible that treated and placebo firms have comparable shock processes. Our identifying restriction is that, conditional on the matched design, the covariance structure of firm-level shocks is the same in the two groups up to a scalar multiplier. Formally, for every pair of outcomes $j$ and $k$, \begin{equation} E^{\mathrm{tr}}\!\left[ e_{it}^{k}e_{is}^{j} \right] = c\, E^{\mathrm{pl}}\!\left[ e_{it}^{k}e_{is}^{j} \right] \qquad \text{for every }t,s, \label{eq:proportional_shock_covariance} \end{equation} for some constant $c>0$, where ``tr'' and ``pl'' denote the treated and placebo samples. We refer to this restriction as the \emph{proportional shock covariance restriction}. It allows firms that change CEOs to have systematically larger or smaller shocks than firms that do not change CEOs. It requires the temporal pattern of variances, autocovariances, and cross-outcome covariances to be the same in the two groups up to the common scale factor $c$. The restriction must cover covariances across different outcomes because the variable used to measure the CEO-spell change need not be the same as the outcome whose response is being studied. For example, suppose that the CEO-spell change is measured using revenue, while the outcome of interest is employment. The covariance bias then depends on the correlation between the revenue shocks entering the estimated CEO-spell change and the employment shocks entering the outcome contrast. We therefore require cross-outcome covariances, as well as within-outcome autocovariances, to be proportional between treated and placebo firms. When $j=k$, the restriction reduces to proportionality of the autocovariance structure of a single outcome. The artificial transition in a placebo firm divides one uninterrupted CEO spell into two parts. Under the model in Section~2, there is therefore no true change in the placebo firm's CEO-spell component: \begin{equation} \Delta z_n^k=0. \label{eq:placebo_true_change} \end{equation} Nevertheless, the two artificial spell averages generally differ because they contain different realizations of firm-level shocks. The estimated placebo change is therefore \begin{equation} \Delta\widehat z_n^k = \eta_n^k. \label{eq:placebo_estimated_change} \end{equation} Consequently, the variance and covariance calculated in the placebo sample contain only the short-spell bias. Under the proportional shock covariance restriction and a common distribution of designs, \begin{equation} \widehat{\operatorname{Cov}}^{\mathrm{pl}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right) \overset{p}{\longrightarrow} \frac{1}{c}A_h^{jk}, \label{eq:placebo_covariance} \end{equation} and \begin{equation} \widehat{\operatorname{Var}}^{\mathrm{pl}}\!\left( \Delta\widehat z_n^k \right) \overset{p}{\longrightarrow} \frac{1}{c}B^k. \label{eq:placebo_variance} \end{equation} The treated moments contain both the economic object of interest and the short-spell bias, whereas the corresponding placebo moments contain only the bias. Scaling the placebo moments by $c$ therefore recovers the bias terms that contaminate the treated moments. The scale factor $c$ captures systematic differences in shock volatility between firms that change CEOs and firms that do not. We estimate $c$ using pre-transition second moments, before the incoming CEO's spell component can affect the firm's outcomes. In the application, we use a minimum-distance estimator that selects $c$ to align a set of treated and placebo pre-transition moments. Under the proportional shock covariance restriction, this estimator converges to the relative scale of the treated and placebo shock processes. \paragraph{Debiased moments.} Subtracting the scaled placebo covariance from the treated covariance gives \begin{align} \widehat{\operatorname{Cov}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right) &= \widehat{\operatorname{Cov}}^{\mathrm{tr}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right) \nonumber\\ &\quad - \widehat c\, \widehat{\operatorname{Cov}}^{\mathrm{pl}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right). \label{eq:debiased_covariance} \end{align} Similarly, the debiased variance is \begin{equation} \widehat{\operatorname{Var}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k \right) = \widehat{\operatorname{Var}}^{\mathrm{tr}}\!\left( \Delta\widehat z_n^k \right) - \widehat c\, \widehat{\operatorname{Var}}^{\mathrm{pl}}\!\left( \Delta\widehat z_n^k \right). \label{eq:debiased_variance} \end{equation} Under strict exogeneity, the proportional shock covariance restriction, a common distribution of designs, and consistency of $\widehat c$, these estimators converge to the corresponding moments involving the true CEO-spell change: \begin{equation} \widehat{\operatorname{Cov}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right) \overset{p}{\longrightarrow} \operatorname{Cov}\!\left( \Delta z_n^k, \Delta_h y_n^j \right), \label{eq:debiased_covariance_plim} \end{equation} and \begin{equation} \widehat{\operatorname{Var}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k \right) \overset{p}{\longrightarrow} \operatorname{Var}\!\left( \Delta z_n^k \right). \label{eq:debiased_variance_plim} \end{equation} The important feature of these results is that consistency does not require CEO spells to become long. The number of observations within each spell may remain fixed while the number of treated and placebo transitions grows. The placebo sample identifies the nonvanishing short-spell bias that would otherwise remain in the probability limits of the treated moments. For regression coefficients and other nonlinear functions of second moments, we first debias the underlying moments and then form the relevant statistic. The debiased projection coefficient is \begin{equation} \widehat\beta_{h,\mathrm{db}}^{jk} = \frac{ \widehat{\operatorname{Cov}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k, \Delta_h y_n^j \right) }{ \widehat{\operatorname{Var}}^{\mathrm{db}}\!\left( \Delta\widehat z_n^k \right) }. \label{eq:debiased_beta} \end{equation} Thus, the correction is performed at the level of the covariance and variance. In general, it is not equivalent to subtracting a placebo regression coefficient from the treated regression coefficient. Correlations, $R^2$ statistics, and other functions of second moments are constructed in the same way: we first correct each underlying variance and covariance and then apply the nonlinear transformation. The placebo-control approach has three advantages. First, it does not require a parametric model for the full covariance structure of firm-level shocks. Second, by reproducing the treated sample's spell lengths, transition dates, event-time contrasts, and aggregation weights, it accommodates short and heterogeneous CEO spells and unbalanced panels. Third, it applies separately at each event time, allowing us to correct both spurious pre-trends and distorted post-transition dynamics.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Database Construction and Descriptive Statistics}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Our analysis combines two administrative datasets. The Firm Registry is maintained by Hungarian Corporate Courts and contains records on all individuals authorized to act on behalf of firms. These records include CEOs and other executives with signatory rights, tracked through a temporal database where each entry reflects representation status over specific time intervals. Updates occur when positions change, personal identifiers are modified, or reporting standards evolve. The registry provides names, addresses, dates of birth (from 2010), and mother’s names (from 1999), though numerical identifiers exist only from 2013 onward. Information on CEOs is linked with tax identifiers to the Balance Sheet Database (maintained by the National Tax and Customs Administration), which has annual balance sheets and income statements, as well as employment, industry code and ownership (state, domestic, foreign) for all Hungarian firms with double-entry bookkeeping. The two datasets cover 1,063,172 firms for the period between 1992 and 2024, yielding 9,627,484 firm-year observations before sample restrictions.

Finding CEOs among all individuals from the Firm Registry requires heuristics since job titles are inconsistently recorded. When explicit “managing director” titles are recorded, we use them directly. For remaining cases, we assume all representatives are CEOs if three or fewer exist at the firm. When more than three representatives are present, we assign CEO status based on continuity with previously identified CEOs. Time spans between appointments are often unclosed or non-contiguous. In these cases we use information from adjacent years to identify the CEO. In all other cases we do not identify a CEO in that firm-year (ADD PROPORTION).
...
We apply several restrictions to create a sample suitable for the analysis. We drop firm-year without a CEO and with more than 12 CEO spells during the observed period to reduce noise from misclassified transitions. We drop firms ever having more than 2 simultaneous CEOs to avoid complex governance structures that complicate identification. We drop the finance sector due to specialized accounting frameworks and regulatory environments. We drop the first year of firms to avoid mixing stock variables with flow variables observed over less than 12 months. Finally, we drop firms which never reach 3 employees during the observation period. This filter removes a substantial portion of observations but eliminates shell companies, tax optimization vehicles, and self-employment arrangements masquerading as corporations. The remaining firms represent genuine businesses with meaningful economic activity where management quality affects performance.
...
Appendix Table \ref{tab:year_desc} presents the number of firms and their average employment in our sample for each year. The number of Hungarian firms increases abruptly during the nineties, from 52 thousand in 1992 to 168 thousand in 2000. The number of firms was quite stable during the next 23 years, and it grew again as the country went out of the great recession of 2008. In 2023 (the last year of our data), the number of firms was over 205 thousand. The average employment size was rather large in early years (19) as it was strongly influenced by the large socialist companies. But employment size quickly declined as small firms entered the data: by 1997 it fell to 9 and remained around this number throughout of the subsequent period.

The table also shows the number of firms with a CEO replacement. Depending on the year, we observe between 3,500 and 25,000 CEO switches, corresponding to a proportion of firms with a CEO replacement of approximately 5\% to 9\%. In total, we have XXX CEO switches, which provides a solid basis for estimating the effect of CEO changes.

%The table also shows the number of CEOs each year, which presents a similar pattern; in 1992 we observe 32 thousand CEOs and in 2022 122 thousand. The total number of distinct CEOs is 340 thousand. 

ÁTÍRNI Appendix Table A2 shows the industrial distribution of firms and CEOs. The largest sectors are Wholesale-Retail, Telecom and Business Services and Nontradable Services.

Firms which experience CEO transitions are different from those which have only one CEO during their whole observed life span. As Table \ref{tab:switch_desc} shows that firms which switch CEOs, are larger both employment (9.5 vs. 2.6), have their output almost twice as large, and 12\% higher output per worker. Return on assets, on the contrary, is smaller by 14\%. The table also shows the changes of these variables around a CEO transition (measured as the proportional change of the average value of the variable in the subsequent and precedent 4 years. ÁTÍRNI
...
DISCUSS TABLE 2: PROPORTION OF FIRMS WITH 2 CEOS, AVERAGE NUMBER OF CEO SWITCHES BY FIRM, TENURE LENGTH OF 1ST, 2ND ETC CEOS. 

The tenure of the firs CEO of the firm is shown in Figure \ref{fig:tenure}. The top panel shows the distribution of tenure at a given year. Not surprisingly, where short tenures abound: 18\% of CEOs have a one-year long tenure, and 12\% two-year long tenure in a given year. The distribution completely changes if we replace the actual tenure with the maximum tenure for each CEO.\footnote{This is technically the maximum tenure of the CEO weighted by the number of firm served.} This statistic shows the prevalence of CEOs in the data serving a given tenure. The bottom panel of Figure \ref{fig:tenure} shows that the typical CEO serves between 4 and 10 years. In this range each tenure has a share of 5-6\% of all firm-years. The prevalence of longer tenures than 10 years gradually decreases up to 20 years, when it levels again. 
...
The first question in the analysis of the value of the CEO (or the effect of CEO change) is the choice of the variable which is used to measure it. The firm's owners may have various objectives: they may want to raise revenue but also profits. From the society's point of view, productivity is arguably the most important firm attribute. Therefore, we carry out the analysis using these three variables as the metric of CEO value. Multiple metrics make possible assessing the activity of various types of CEOs: depending on the metric what firm attributes do they increase the most?

Table \ref{tab:table_application} presents the naive and debiased estimated coefficients and the $R^2$ of the estimation for output (measured by log sales and an exporting dummy variable), inputs (log capital and labor) and financial and operating productivity (return on assets and labor productivity). The naive and debiased regression coefficients are usually very similar, which is due to the fact that both the covariance between the dependent and the explanatory variable and the variance of the explanatory variable are upward biased. We debias both and the result is many times small. Nonetheless in some cases the debiased coefficient is substantially smaller than the naive coefficient. For example, the change in log revenue is 12 log point lower when revenue itself is the metric, the increase in ROA is 11 log point lower when ROA is the metric and employment raises by 8.4 log points more and labor productivity by 15 log point less when labor productivity is the metric. 

Contrary to the coefficients, the $R^2$ of the estimation is much smaller in many cases. On average, the value from the naive estimation is 2.8 times larger than from the debiased estimation (we did not take into account the $R^2$-s which are essentially zero). In magnitude the difference is also sizable: in those cases when the naive $R^2$ is at least 0.05, the naive estimate is smaller by 0.16 on average. In three cases the difference is close to 0.5. 

Figures \ref{fig:application_lnR} to \ref{fig:application_lnYR} show the event-time studies for the same metrics and dependent variables (the reference year is one year before the CEO transition). The red lines represent the coefficients from the naive estimation and the blue lines from the debiased estimation. The most important message of the figures is that the pre-trend largely diminishes when the placebo controlled method is used. Visual inspection of the figures demonstrates that in 10 out of 18 cases, the pre-trend is considerably smaller than with the naive estimation, or disappears completely. In only one case it becomes larger while in the remaining 7 cases they are very similar. This is very important because the quality of the estimation is decided upon pre-trends. Our exercise demonstrates that the limited mobility bias may show up in way which may make researchers reject well specified estimations. 

The differences between the estimated coefficients under the two methods for the period subsequent to the CEO transition are less dramatic. In accordance to the average effects presented in Table \ref{tab:table_application}, the two lines mostly differ in their shape and the in magnitude. In several cases, however, the debiased estimations carry on very a different message regarding firm behavior. For example, the naive estimates suggest that capital falls when ROA is the measure of quality of CEO change, but the naive estimations reduce this to nothing. The trajectory of employment is also very different across the two specification when labor productivity is the metric: while the naive resuls suggest a dip in the first year under the new CEO and then some increase, the debiased method estimates are substantial increase which reaches 13 log points after 4 years. Labor productivity estimates are similar in their shape, but not in magnitude: the variable increases by 80 log points in the naive estimation and 55 log point when the placebo controlled debias is applied. 

Taken together, the comparison of the naive ad debiased result show that there is nothing mechanical in our method in the sense that the estimated coefficients vary widely depending on the metric and dependent variable. Sometimes they are very similar; sometimes only the pre-trend disappears, in some cases they have similar shape but the magnitude differs; and sometimes thy suggest completely different trajectories of the dependent variables.

The three metrics of CEO change enable us to draw conclusions about what can stakeholders expect from various CEO types? For example, what is the difference between a CEO who is good at increasing sales and one who boosts the return on assets? The coefficients can be interpreted as elasticities (semielasticities in the case of the export dummy and ROA) of the dependent variable with respect of our measure of CEO change (which is measured in the metric used). Table \ref{tab:table_application} reveals that those CEOs who raise sales, do raise every outcome we analyze. All the elasticities are large and statistically significant. One standard deviation higher $\Delta z$ induces 66\% increase in revenue. It also increases labor and capital (the corresponding elasticities equal 0.23 and 0.34). The revenue increase is accompanied by substantial increases in productivity: the elasticity of ROA is 0.80 and of labor productivity 0.30. 

Those CEOs who are good at increasing labor productivity, raise sales, exports (though not as much as in those who raise sales), capital but not labor, and ROA (this latter is associated with a very large, but statistically insignificant coefficient). Those who increase ROA do not raise the scale of production, nor exports and labor productivity. In conclusion, it seems that the best CEOs are those who concentrate on raising the volume of sales, as this raises both inputs and also productivity. 

The $R^2$ estimations suggest that CEO change explain less of the variance of the dependent variables as one would expect (REF EARLIER STUDIES). When the metric is the value of sales $R^2$ is larger than 0.10 only when it is estimated on the metric itself. For labor is it 0.09 and for all other dependent variables less than 0.04. When ROA is the metric, the $R^2$s are all close to zero (except for ROA, when it is 0.13). It seems, thus, that CEO change cannot explain itself changes taking place in the firm. (EZ VAJON IGAZ? ÉS HA IGEN, AKKOR MI LEHET AZ OKA? AZ, HOGY JÓ CÉGEKNEK JÓ CEO-JA VAN ÉS AKI LECSERÉLI, AZ IS JÓ? VAGYIS A DELTA Z NEM MAGAS? MEG KELLENE MUTASSUK A DELTA Z STATISZTIKÁIT)   
...
\begin{table}[h]
    ...
    \begin{minipage}{16cm}
    \vspace{.2cm}
    \footnotesize{Notes: The table presents the estimated naive and placebo debiased coefficients corresponding to the CEO change $\Delta z$ and the values of the corresponding $R^2$ on a simulated sample of 50,000 treated firms and 50,000 control firms. The treatment (placebo treatment) year is drawn from a geometric distribution with a constant hazard of CEO change in the years $T = 2...6$. The standard deviation of $\Delta z = 1$. In the baseline scenario the standard deviation of errors $\sigma(\epsilon) = 0.71$ in the treated and control groups. Each firm has a spell $T_{max} = 10$ time periods. The error autocorrelation $\rho = XXX$. Column 1: baseline scenario; Column 2: the treated group's error has an excess variance of $\sigma(\epsilon) = 1.2 \cdot 0.71$; Column 3: Excess variance correction is applied; Column 4: firm spells are raised to $T_{max} = 20$. $R^2$ is from the estimation based on all years (ATET); $R^2 \text{ at } t =3)$ is from an estimation based on the difference between three years after relative to the year precedent to the treatment.}
    \end{minipage}
\end{table}
\vspace{.5cm}
...
\begin{table}[H]
    \centering
    \caption{Switch Characteristics}
    \vspace{.2cm}
    ...
    \begin{minipage}{16cm}
    \vspace{.2cm}
    \footnotesize{Notes: this table shows the characteristics of firms with and without CEO replacement and the change of firm characteristics around the CEO replacement.}
    \end{minipage}
\end{table}
\vspace{.5cm}
...
\begin{table}[H]
    \centering
    \caption{Characteristics of CEO Tenure}
    \vspace{.2cm}
