...
The source of this problem is the short length of CEO spells. With only a small number of annual observations in each spell, the estimated CEO-spell component reflects a mixture of the true CEO-spell component and firm-level shocks. The resulting estimation error may be correlated with the outcome contrast, causing the covariance between the measured CEO-spell change and the outcome to differ systematically from the covariance involving the true CEO-spell change. As spell lengths increase and firm-level shocks average out, this source of covariance bias disappears.
...
covariance structure of firm-level shocks. The key insight is that the
short-spell bias depends on two objects: how the estimated CEO-spell
...
restriction.}
For each firm that moves from CEO spell $s-1$ to spell $s$, we select
one or more observably similar firms whose CEO does not change over the
relevant event window. We match firms on characteristics such as sector
and birth cohort. We assign each matched firm a placebo transition in
the same calendar year $g_{is}$ as the corresponding actual transition
and divide its uninterrupted CEO spell into artificial pre- and
...
event-time comparisons to construct $\Delta_h q_{is}^j$. Assigning the
placebo transition in the same calendar year also ensures that treated
and placebo firms are exposed to the same aggregate time period.
...
treated and placebo firms have comparable shock processes. Our
identifying restriction is that, conditional on the matched design, the
covariance structure of firm-level shocks is the same in the two groups
up to a scalar multiplier. Formally, for every pair of outcomes $j$
and $k$,
\begin{equation}
...
\end{equation}
for some constant $c>0$. We refer to this restriction as the
...
sample contain only the short-spell bias. Under the proportional shock
covariance restriction and a common distribution of designs,
\begin{equation}
...
contain only the bias. Scaling the placebo moments by $c$ therefore
recovers the bias terms that contaminate the treated moments.

The scale factor $c$ captures systematic differences in shock
volatility between firms that change CEOs and firms that do not. We
estimate $c$ using pre-transition second moments, before the incoming
CEO-spell component can affect the firm's outcomes. In the application,
we use a minimum-distance estimator that selects $c$ to align a set of
...
a common distribution of designs, and consistency of $\widehat c$,
...
lengths, transition dates, event-time contrasts, and aggregation
weights, it accommodates short and heterogeneous CEO spells and
