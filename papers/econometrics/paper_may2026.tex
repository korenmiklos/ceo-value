...
The numerator of the naive estimator is also affected by the
...
observations, so firm-level shocks do not average out. Moreover, the
estimated explanatory variable is constructed from the firm's own
realized outcomes. Some of the same shocks may therefore affect both
the estimated CEO-spell change and the outcome contrast, creating a
mechanical correlation between them.
...
shocks are serially correlated, shocks used to estimate the component
associated with the incoming CEO spell may be correlated with firm
outcomes observed before the CEO arrives.
...
Short-spell estimation error affects any statistic constructed from
second moments of estimated CEO-spell changes.

First, the variance of the estimated CEO-spell changes overstates
genuine heterogeneity in the changes experienced by firms:
\[
\operatorname{Var}\!\left(
\Delta\widehat z_{is}^{k}
\right)
...
covariance distortions through the ratio in
Equation~\eqref{eq:naive_beta_plim}. The resulting asymptotic bias may
be upward or downward.

Fourth, correlations and $R^2$ statistics are nonlinear functions of
variances and covariances. Correcting only one component is therefore
insufficient: the underlying second moments must be corrected before
...
test may therefore reject the null of no pre-trends because of the
mechanical relationship between the estimated CEO-spell change and
persistent firm-level shocks, rather than because the true CEO-spell
change predicts outcomes before the transition.
