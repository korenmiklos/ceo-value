...
Our corporate data cover the period 1992–2022 (31 years). We restrict the sample to firms that have ever reached at least 5 employees in their lifetime, to exclude non-employer businesses that rarely change CEOs and would inflate the sample numerically without adding identifying transitions. We drop firms from mining and finance due to specialized accounting frameworks and regulatory environments. The resulting sample has 113,918 firms with a mean employment of 11.5 and average lifespan of 12.8 years. They are served by 160,188 CEOs. The treated group in our sample includes firms that have changed CEOs at least once (42,511 firms, 93,666 CEOs, average employment 14.8, and average lifespan 14.1 years). To highlight the paucity of CEO mobility, note that 84\% of the switching CEOs have served at only one firm, and only 3.6\% served at more than two firms. The control group includes a random sample of all non-changers as well as non-changing periods of treated firms, with exact matching on industry, year, and firm age.

In this exercise, our goal is to estimate the effect of CEO changes on firm growth, which we proxy by firm revenue. We remove industry-year means so that the outcome is measured as a deviation from the sectoral environment. Because applied work on firm-level studies commonly examines pre-trends to assess the plausibility of the estimates and the time evolution of effects, we present event-study profiles. 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{\figurepath/figuremc.pdf}
...
\vspace{.5cm}
    ...
    \centering
    \caption{Firm-Level Descriptive Statistics
    \vspace{.2cm}
    \label{tab:firm_desc}
