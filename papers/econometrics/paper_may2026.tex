...
We impose strict exogeneity. Let $\mathcal S_i$ denote the set of CEO spells observed at firm $i$.
...
For firm $i$, consider two adjacent CEO spells, $s-1$ and $s$. The
...
$s-1$ to spell $s$. Let $q_{it}^{j}$ denote outcome $j$ of firm $i$ in
...
other exactly, the naive estimator converges to a value different from
$\beta_h^{jk}$.
...
The noise in the estimated CEO-spell change is the difference between
the average firm-level shocks in spells $s$ and $s-1$. Let the variance
...
This variance distortion does not disappear simply because the number
of firms grows. If spell lengths remain bounded, then, as the number of
observed adjacent spell pairs grows,
\begin{equation}
\widehat{\operatorname{Var}}\!\left(
\Delta\widehat z_{is}^{k}
\right)
...
estimation error converges to zero---for example, as the number of
