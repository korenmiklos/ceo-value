...
[[Krisztina: For \ref{tab:sample} I hadn't realized how small the largest conn. component is relative to the full sample. You mentioned the second largest conn. component is much smaller than the largest conn. comp, right?]
...
[[Krisztina: Is it not a problem that when we get to the measurement part, we measure the manager effect differently than what we say in the modelling part? I think this might be a relevant distinction between whether the manager's skill is measured as just average non time varying residual productivity beyond firm FE vs average non time varying residual productivity beyond beyond firm-manager effects.I think either way is fine, but i think we need to be more clear in the modelling section on what we will end up measuring? What do you think? ]]
...
%[[Krisztina: I think (2) is too weak: uncorrelatedness does not imply zero conditional  mean, and that's what we need for OLS to be unbiased. Also, I am not sure there is a problem with conditioning on the fixed effects. When we write down a fixed effects model, obtaining an unbiased estimate of the coefficients necessitates the strict exogeneity assumption: $E[\tilde{\epsilon}_{it}| k_{it}, z_m, \lambda_i, \mu_{st}]=0$ This means that ]]
