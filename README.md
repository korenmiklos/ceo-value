...
| `temp/manager_value.dta` | Derived | Manager fixed effects and skill estimates | Generated by code |
| `temp/revenue_models.ster` | Derived | Saved revenue function estimation results | Generated by code |
| `temp/placebo_*.dta` | Derived | Event study samples (full, fnd2non, non2non, post2004) | Generated by code |
...
- Data processing is sequential: balance.do → ceo-panel.do → unfiltered.do → analysis-sample.do → event_study_sample.do
...
- `code/create/event_study_sample.do`: Generates placebo CEO transitions with same probability as actual changes but excluding actual transition periods, creating separate samples for different event study specifications
...
- `code/estimate/event_study.do`: Implements placebo-controlled event study comparing actual vs placebo CEO transitions for multiple outcomes and samples
- `code/estimate/setup_event_study.do`: Helper script that sets up event study specifications and definitions
- `code/estimate/anova.do`: Implements ANOVA decomposition of treatment effects
- `code/estimate/setup_anova.do`: Helper script for ANOVA analysis setup
...
- `code/exhibit/tableA0.do`: Creates Table A0 - Bloom et al. (2012) autonomy analysis (appendix)
- `code/exhibit/tableA1.do`: Creates Table A1 - Industry-level summary statistics (appendix)
- `code/exhibit/figure2.do`: Creates Figure 2 - Event study by CEO transition type (four panels)
- `code/exhibit/figure3.do`: Creates Figure 3 - Event study outcomes (Capital, Intangibles, Materials, Wagebill)
- `code/exhibit/event_study.do`: Helper script for event study figure generation
...
5. **Alternative manual execution**: If Make is not available, refer to the Makefile for the complete dependency chain. Key steps include:
   ```bash
   ...
   # Generate event study samples and results (for each sample: full, fnd2non, non2non, post2004)
   stata -b do code/estimate/event_study.do <sample> <outcome>
   
   # Create exhibit tables and figures
...
| Table A0          | code/exhibit/tableA0.do  | output/table/tableA0.tex         | Bloom et al. (2012) autonomy analysis (appendix) |
| Table A1          | code/exhibit/tableA1.do  | output/table/tableA1.tex         | Industry-level summary statistics (appendix) |
| Figure 2          | code/exhibit/figure2.do  | output/figure/figure2.pdf        | Event study by CEO transition type (four panels) |
| Figure 3          | code/exhibit/figure3.do  | output/figure/figure3.pdf        | Event study outcomes (Capital, Intangibles, Materials, Wagebill) |
...
| Event Study Data  | code/estimate/event_study.do | output/event_study/*.csv | Event study coefficients for all samples and outcomes |
