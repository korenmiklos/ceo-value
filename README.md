...
| `output/extracts/*.dta` | Derived | **Confidential** data extracts for external analysis | Not provided |
...
The project creates three confidential data extracts in `output/extracts/` for external analysis. These extracts contain sensitive firm and manager information and are **not included in the repository** (see `.gitignore`).

### Extract 1: 2022 Manager Values
- **File**: `output/extracts/extract1_2022_values.dta`
- **Content**: Firm fixed effects and manager values for year 2022
- **Variables**: `frame_id`, `firm_fixed_effect`, `manager_value`

### Extract 2: Manager Changes in 2015
- **File**: `output/extracts/extract2_manager_changes_2015.dta`
- **Content**: Firms with CEO transitions in 2015, comparing performance before and after
- **Sample**: Single-CEO firms where the pre-2015 manager was stable (2012-2013) and post-2015 manager was stable (2015-2017)
- **Variables**: `frame_id`, `lnStilde_before` (mean 2012-2013), `lnStilde_after` (mean 2016-2017)

### Extract 3: Connected Component Managers
- **File**: `output/extracts/extract3_connected_managers.dta`
- **Content**: Manager characteristics for those in the largest connected component
- **Variables**: `person_id`, `manager_value`, `gender`, `birth_year`, `first_ceo_year`

To generate these extracts, run:
```bash
make output/extracts/extract1_2022_values.dta
```
