...
- Stata (code was last run with version 18.0)
  - No additional packages required beyond standard Stata installation
...
The code was last run on a **MacOS machine with Stata 18.0**. Processing involves substantial data manipulation of millions of firm-year observations, requiring adequate RAM and processing power.
...
- `output/paper.pdf` depends on `output/paper.tex`
...
4. **Alternative manual execution**: If Make is not available, run the following Stata commands from the project root:
   ```bash
...
- The final analytical sample contains 8,872,039 firm-year observations
